...
def process_worksheet_node(
    buffer: bytes, state: SheetStreamingState, processed_tags: Set[bytes]
) -> CollectChunksResult:
    if state != SheetStreamingState.IN_WORKSHEET_NODE:
        raise ValueError(f"Invalid state {state} for process_worksheet_node")
    ...
    remaining_tags_to_process: List[TagToProcess] = [
        tag for tag in tags_to_process_in_order if tag not in processed_tags
    ]
    for tag in remaining_tags_to_process:
        ...
        if not tag.tag_end:
            ...
            found_tag = find_and_slice_after_tag_range(
                buffer, tag.tag_start, tag.tag_end
            )

        if found_tag and not found_tag.is_self_closing_tag:
            new_state = (
                SheetStreamingState.IN_COLS_NODE
                if tag == cols_tag
                else (
                    SheetStreamingState.IN_SHEET_DATA_NODE
                    if tag == sheet_data_tag
                    else (
                        SheetStreamingState.IN_MERGECELLS_NODE
                        if tag == merge_cells_tag
                        else (
                            SheetStreamingState.FOUND_WORKSHEET_END
                            if tag == worksheet_end_tag
                            else None
                        )
                    )
                )
            ...
            if new_state is None:
                raise ValueError(f"Invalid tag {tag} found in worksheet node.")

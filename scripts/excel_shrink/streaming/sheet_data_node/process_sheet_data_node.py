from common.named_tupples import CleanSheetDataResult, CleanSheetParameters, XF, CellCoordinate
...
from cleaning.generate_and_log_rows_and_cells import generate_and_log_deleted_cell, generate_and_log_self_closing_cell
...
def find_sheet_data_chunk(buffer: bytes) -> SliceAtTagResult | None:
    found_last_row_start_tag = find_and_slice_before_tag(buffer, b"<row", search_backwards=True)
...
def remove_consecutive_empty_cells(chunck, params):
    if b'<c' in chunck:
        chunck = params.workbook_specific_patterns.pattern_consecutive_only_font_styled_empty_cells.sub(b'', chunck)
    return chunck
...
def cell_overrides_fill_or_border(cell_style: XF, cell_coordinate: CellCoordinate, metrics: CleanSheetMetrics) -> bool:
    column_index = letters_byte_string_to_integer(cell_coordinate.column)
    ...
    overrides_fill_or_border = (
        (
            (row_style and row_style.apply_fill and cell_style.fill_id != row_style.fill_id) or
            (column_style and column_style.apply_fill and cell_style.fill_id != column_style.fill_id)
        )
        or
        (
            (row_style and row_style.apply_border and cell_style.border_id != row_style.border_id) or
            (column_style and column_style.apply_border and cell_style.border_id != column_style.border_id)
        )
    )
...
def replace_cell(match: re.Match, params: CleanSheetParameters, metrics : CleanSheetMetrics) -> bytes:
...
def process_sheet_data_node(buffer: bytes, params : CleanSheetParameters) -> CleanSheetDataResult | None:
    ...
    if found_closing_sheet_data_tag:
        ...
        return CleanSheetDataResult(
            ...
            chunk_to_add=pre_process_sheet_data_chunk(found_closing_sheet_data_tag.until_tag, params),
    ...
    return CleanSheetDataResult(
        ...
        chunk_to_add=pre_process_sheet_data_chunk(find_sheet_data_chunk_result.until_tag, params),

...
def generate_whitespace_cell_pattern(
    whitespace_indices_joined: bytes | None,
) -> re.Pattern | None:
...
def generate_whitespace_pattern(
    whitespace_indices_joined: bytes | None,
) -> re.Pattern | None:
    if not whitespace_indices_joined:
        return None
...
def generate_whitespace_joined(whitespace_indices: List[int] | None) -> bytes | None:
    if not whitespace_indices or len(whitespace_indices) == 0:
        return None
...
def find_no_style_but_font_ids(all_xfs: List[XF]) -> set[int]:
    ...
    return {
        i
        for i, xf in enumerate(all_xfs)
        if (
            (xf.fill_id is None or xf.fill_id == 0)
            and (xf.border_id is None or xf.border_id == 0)
            and not xf.apply_fill
            and not xf.apply_border
            and not xf.has_apply_protection
        )
    }


def generate_pattern_consecutive_only_font_styled_empty_cells(
    only_font_style_ids_joined: bytes | None,
) -> re.Pattern:
...
def generate_pattern_remove_irrelevant_attributes_for_empty_rows(
    only_font_style_ids_joined: bytes | None,
) -> re.Pattern:
    if only_font_style_ids_joined is None:
...
def generate_only_font_style_ids_joined(xfs: List[XF]) -> bytes | None:
    ...
    only_font_style_bytestring_ids = [
        str(id).encode("ascii") for id in only_font_style_ids
    ]
...
def find_fill_or_border_or_apply_protection_ids(all_xfs: List[XF]) -> set[int]:
    return {
        i
        for i, xf in enumerate(all_xfs)
        if (xf.fill_id is not None and xf.fill_id != 0)
        or (xf.border_id is not None and xf.border_id != 0)
        or (xf.has_apply_protection)
    }


def generate_fill_or_border_or_protection_or_hidden_attribute_pattern(
    fill_or_border_or_protection_bytestring_ids: List[bytes],
) -> re.Pattern:
    fill_or_border_or_protection_bytestring_ids_joined = b"|".join(
        fill_or_border_or_protection_bytestring_ids
    )

    fill_or_border_or_hidden_attribute_pattern = re.compile(
        rb'(?:s="(?:'
        + fill_or_border_or_protection_bytestring_ids_joined
        + rb')"|"hidden="1")'
    )
...
class WorkbookSpecificPatterns(NamedTuple):
    ...
    fill_or_border_or_protection_or_hidden_attribute_pattern: re.Pattern
    whitespace_pattern: re.Pattern | None
    whitespace_cell_pattern: re.Pattern | None


def generate_workbook_specific_patterns(
    xfs: List[XF], whitespace_indices: List[int] | None
) -> WorkbookSpecificPatterns:
    ...
    fill_or_border_or_protection_ids = find_fill_or_border_or_apply_protection_ids(xfs)
    fill_or_border_or_protection_bytestring_ids = [
        str(id).encode("ascii") for id in fill_or_border_or_protection_ids
    ]

    only_font_style_ids_joined = generate_only_font_style_ids_joined(xfs)
    pattern_remove_irrelevant_attributes_for_empty_rows = (
        generate_pattern_remove_irrelevant_attributes_for_empty_rows(
            only_font_style_ids_joined
        )
    )
    pattern_consecutive_only_font_styled_empty_cells = (
        generate_pattern_consecutive_only_font_styled_empty_cells(
            only_font_style_ids_joined
        )
    )
    fill_or_border_or_protection_or_hidden_attribute_pattern = (
        generate_fill_or_border_or_protection_or_hidden_attribute_pattern(
            fill_or_border_or_protection_bytestring_ids
        )
    )

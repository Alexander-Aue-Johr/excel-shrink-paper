...
def collect_col_chunks_until_first_extraneous_column(cols_content: bytes, last_max_col: int) -> tuple[re.Match[bytes] | None, bytes]:
    col_chunks : bytes = b''
...
def merge_extraneous_columns(cols_content: bytes, last_max_col: int | None) -> bytes:
    ...
    first_extraneous_column_match, col_chunks = collect_col_chunks_until_first_extraneous_column(cols_content, last_max_col)

    if not first_extraneous_column_match:
        logging.debug('no extraneous columns found')
        return cols_content

    last_column_match = pattern_last_cols.search(cols_content, first_extraneous_column_match.end())
    if not last_column_match:
        logging.debug('the last column already is the first extraneous column')
        return cols_content
    
    first_extraneous_column_max_attribute_string = first_extraneous_column_match.group(2)
    last_column_max_attribute_string = last_column_match.group(2)

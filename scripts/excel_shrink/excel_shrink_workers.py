...
from cleaning.clean_empty_rows_from_after_cell_range import (
    clean_empty_rows_from_after_cell_range,
)
...
from cleaning.generate_workbook_specific_patterns import (
    generate_workbook_specific_patterns,
)
from common.letters_byte_string_to_integer import letters_byte_string_to_integer
from parsing.parse_xfs import parse_xfs
from cleaning.clean_sheet_rows_and_cells_in_cell_range import (
    clean_sheet_rows_and_cells_in_cell_range,
)
...
from optimized_regex_excel_shrink.find_whitespace_sst_indices import (
    find_whitespace_sst_indices,
)
from optimized_regex_excel_shrink.clean_workbook import clean_workbook
from streaming.clean_sheet_xml_streamwise import (
    preprocess_sheet_xml_streamwise,
)


def clean_xml_data(
    params: CleanWorkbookParameters | CleanSheetParameters,
    chunk_size: int,
    ...
    with zipfile.ZipFile(params.xlsx_file_path, "r") as z:
        ...
        if isinstance(params, CleanSheetParameters):
            ...
            with z.open(params.zip_info.filename) as xml_data_stream:
                ...
                if not disable_sheetdata_splitting:
                    ranges = retrieve_sheet_data_range_containing_cells(
                        preprocessed_sheet_xml.sheet_data, params
                    )

                    cleaned_cells_range = clean_sheet_rows_and_cells_in_cell_range(
                        ranges.cells_range, params, metrics
                    )
                    cleaned_visible_rows_range = clean_empty_rows_from_after_cell_range(
                        ranges.rows_range, params, metrics
                    )
                else:
                    cleaned_cells_range = clean_sheet_rows_and_cells_in_cell_range(
                        preprocessed_sheet_xml.sheet_data, params, metrics
                    )
                    cleaned_visible_rows_range = b""

                final_sheet_data = cleaned_cells_range + cleaned_visible_rows_range

                max_column = get_max_column(metrics)
                in_cols_node_output_chunks = merge_extraneous_columns(
                    preprocessed_sheet_xml.in_cols_node_output_chunks, max_column
                )

                final_data = (
                    preprocessed_sheet_xml.until_worksheet_start_output_chunks
            ...
            with z.open(params.zip_info.filename) as xml_data_stream:
                ...
                clean_xml_result = clean_workbook(xml_data, params)
        ...
        if logging.getLogger().isEnabledFor(logging.INFO):
            ...
            logging.info(
                f"Processed Workbook of {params.xlsx_file_path}: Original size {params.zip_info.file_size} bytes, Clean size {clean_size} bytes, "
...
def find_last_fill_or_border_or_protection_style_or_hidden_attribute(
    sheet_data: bytes, params: CleanSheetParameters, last_row_with_cell_end_pos: int
) -> int:
    fill_or_border_or_protection_or_hidden_attribute_pattern = (
        params.workbook_specific_patterns.fill_or_border_or_protection_or_hidden_attribute_pattern
    )
    last_pos = last_row_with_cell_end_pos

    last_visible_attribute_match = None
    for match in fill_or_border_or_protection_or_hidden_attribute_pattern.finditer(
        sheet_data, last_row_with_cell_end_pos
    ):
...
def find_last_visible_row_end_position(
    sheet_data: bytes, last_visible_attribute_position: int
) -> int:
    if last_visible_attribute_position == -1:
        return -1
...
def find_last_row_with_cell_end_pos(sheet_data: bytes) -> int:
    ...
    if last_cell_pos == -1:
        return -1
    last_row_with_cell_end_pos = sheet_data.find(b"</row>", last_cell_pos) + len(
        b"</row>"
    )
...
def retrieve_sheet_data_range_containing_cells(
    sheet_data: bytes, params: CleanSheetParameters
) -> VisibleSheetDataRanges:
    last_row_with_cell_end_pos = find_last_row_with_cell_end_pos(sheet_data)

    last_fill_or_border_or_protection_style_or_hidden_attribute = (
        find_last_fill_or_border_or_protection_style_or_hidden_attribute(
            sheet_data, params, last_row_with_cell_end_pos
        )
    )
    last_visible_row_end_position = find_last_visible_row_end_position(
        sheet_data, last_fill_or_border_or_protection_style_or_hidden_attribute
    )
...
def find_apply_fill_border_ids(all_xfs: List[XF]) -> set[int]:
    ...
    """
    return {i for i, xf in enumerate(all_xfs) if xf.apply_fill or xf.apply_border}


si_pattern = re.compile(rb"<si>.*?<\/si>", flags=re.DOTALL)
sheet_xml_name_pattern = re.compile(r"^xl/worksheets/sheet[1-9][0-9]*\.xml$")


def generate_clean_sheet_params_for_xlsx_file(
    ...
    with zipfile.ZipFile(file_path, "r") as zin:
        ...
        if remove_whitespaces:
            try:
                ...
                # sharedStrings.xml does not exist in this XLSX
                si_matches = []

            whitespace_indices = find_whitespace_sst_indices(si_matches)
        else:
            whitespace_indices = None

        with zin.open("xl/styles.xml") as styles_file:
            styles_xml = styles_file.read()
            cell_styles, _, _, _ = extract_xml_block(
                styles_xml, b"<cellXfs", b"</cellXfs>"
            )
            all_xfs = parse_xfs(cell_styles) if cell_styles else []

        workbook_specific_patterns = generate_workbook_specific_patterns(
            all_xfs, whitespace_indices
        )
    ...
    return CleanXlsxParameters(
        file_path,
        clean_workbook_parameters,
        clean_sheet_parameters_list,
        workbook_zip_info,
        sheets,
        xml_files_excluded_from_cleaning,
        only_clean_workbook,
    )

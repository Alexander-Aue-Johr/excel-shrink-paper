...
mergecell_pattern = re.compile(
    rb"<mergeCell\b" rb'[^/>]*\bref="([A-Za-z]+)(\d+):([A-Za-z]+)(\d+)"' rb"[^/>]*/>"
)


def find_max_merge_cells_column_and_row_bytestrings(
    merge_cell_data: bytes,
) -> CellCoordinate | None:
    ...
    distinct_columns = {m[0] for m in merge_cell_matches} | {
        m[2] for m in merge_cell_matches
    }
    distinct_rows = {m[1] for m in merge_cell_matches} | {
        m[3] for m in merge_cell_matches
    }
...
def process_merge_cells_node(buffer: bytes) -> MergeCellsParsingResult | None:
    ...
    max_merge_cell = find_max_merge_cells_column_and_row_bytestrings(
        found_closing_tag.until_tag
    )

...
\textit{Excel} allows users to define the area of a worksheet that should be printed on paper. This area can be manually defined or automatically set by \textit{Excel}. In both cases, this area is stored in the \texttt{workbook.xml} file under the \texttt{<definedNames>} node~\cite{microsoft2024workbook}. Typically, this area is labeled as \texttt{\_xlnm.Print\_Area}.
\textit{Excel} also allows users to define an area that should always appear at the top of the printed page, like table headers. These are stored as \texttt{\_xlnm.Print\_Titles}. However, these areas and titles are sometimes also stored under the names \texttt{Print\_Area} and \texttt{Print\_Titles}. When \textit{Excel} opens the file, it automatically renames any \texttt{Print\_Area} and \texttt{Print\_Titles} entries to \texttt{\_xlnm.Print\_Area} and \texttt{\_xlnm.Print\_Titles}, respectively.

This process creates a problem when both variants are already present workbook as seen in Listing~\ref{lst:PrintAreaRedundancy}~\cite[\texttt{xl/workbook.xml}]{destatis2022Wanderungen}. Then, renaming the variant without the leading \texttt{\_xlnm.} to the one with this prefix would result in two definitions of the same name for one worksheeet. In this case, \textit{Excel} does not automatically rename those definitions but rather, presents a dialog to the user upon opening the file, asking the user to select a new name for the \textit{Defined Name}.
...
To spare users the need to manually rename redundant entries, \textit{Excel Shrink} was extended to automatically remove redundant entries.

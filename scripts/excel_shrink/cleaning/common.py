...
from common.named_tupples import XF, CellCoordinate, CleanSheetParameters, ColumnRangeMap
...
def extract_empty_row_attributes (match: re.Match, params : CleanSheetParameters) -> RowAttributes:
...
def extract_row_attributes (match: re.Match, params : CleanSheetParameters) -> Tuple[RowAttributes, bytes]:
    ...
    return RowAttributes(row_attr, ref, is_hidden, style, has_height, is_empty), original_row_content
...
def extract_cell_attributes (match: re.Match, params : CleanSheetParameters) -> CellAttributes:
    cell_attr, col_ref, row_ref, style_id  = match.groups()

    style = params.workbook_specific_patterns.xfs_by_bytestring[style_id] if style_id else None

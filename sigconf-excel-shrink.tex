...
Excel files are widely utilized as a format for data exchange across various domains, including finance, statistics, business, and research \cite{panko1998we}. Their versatility lies in \textit{Excel}'s capability to store tabular data, perform complex calculations, automate workflows using macros, and visually represent data through charts and graphs.~\cite{rahman2021noah} \cite{userExperienceSpreadsheets2022}   This flexibility makes \textit{Excel} indispensable in many organizations. However, as spreadsheets grow in complexity, significant challenges arise regarding file size and performance management, prompting novel solutions \cite{swidan2016improving, tang2023efficient, antiFreezeBendre2019sigmod, bendre2015dataspread}.
...
As part of our evaluation, we processed all \textit{Excel} files~\cite{shrinked_destatis_xlsx_files}
obtained from the German Federal Statistical Office (\textit{Destatis})~\cite{original_destatis_xlsx_files,shrinked_destatis_xlsx_files}. The full details of how these files were acquired through a custom web scraping approach are provided in Appendix~\ref{sec:scraping_destatis}. 
The primary goal of this evaluation is to determine the extent of data reduction achieved through different techniques, including overall reduction, empty cell reduction, whitespace cell reduction, rows outside of value range removal, and column removal. Figure~\ref{fig:size-reduction-chart} illustrates the distribution of reduction rates (in percentage) for individual worksheets across all categories, broken down by Empty Cells, Whitespace Cells, Rows Outside of Value Range, and Removed Columns. The chart indicates that some worksheets experienced reductions of nearly 100\% in file size. Specifically, one worksheet had a reduction rate of over 99.99\%; 16 worksheets exceeded 99.9\%; and 210 worksheets achieved reductions above 99\%. The average reduction rate across all worksheets was 25\%.

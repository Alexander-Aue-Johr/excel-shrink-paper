...
from cleaning.common import (
    CleanSheetMetrics,
    extract_empty_row_attributes,
    previous_row_was_also_hidden,
)
...
from cleaning.generate_and_log_rows_and_cells import (
    generate_and_log_deleted_row,
    generate_and_log_self_closing_row,
)
...
def replace_empty_row(
    ...
    if not row_style or (not row_style.has_fill_or_border and not row_style.has_apply_protection):
        return generate_and_log_deleted_row(metrics, row_attributes)
...
def clean_empty_rows_from_after_cell_range(
    sheet_data: bytes,

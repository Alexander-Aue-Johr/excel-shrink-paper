...
\begin{abstract}
  This paper introduces Excel Shrink, a tool designed to mitigate the pervasive problem of file bloat in Microsoft Excel spreadsheets. By analyzing Excel’s Office Open XML structure, we identify common sources of file bloat—such as cells, rows and columns that are empty, 
  %contain only whitespace, 
  or have a style assigned that is invisible to the user. We propose a novel, streaming-based, regex-driven approach to reduce file size without altering the visual appearance. Our method employs a state-machine to manage the hierarchy of XML documents without relying on conventional XML parsing techniques by processing XML data incrementally at the bytestream level, and it leverages multithreading for performance improvements. This enables Excel Shrink to outperform Microsoft Excel when loading bloated files, and cleaning plus opening the shrunk file is faster than opening the bloated file directly.
  Evaluation on large-scale real-world spreadsheets from the German Federal Statistical Office and data.gov demonstrates that Excel Shrink can achieve file size reductions of up to 99.99\% and dramatically improve file loading times.
  This work not only provides practical insights for developers and end users but also opens new avenues for the efficient management of large-scale semi-structured data in spreadsheet environments.
...
This paper situates the Excel bloat problem within the context of database research
and proposes technical approaches to mitigate it.
We begin by analysing the internal structure of Excel's Office Open XML format
to identify the root causes of file bloat,
and review both related academic work and practitioner-oriented resources
addressing bloat in Excel files.
To ground this analysis, we examine manual guidelines as well as tools such as
Microsoft Excel’s Performance Tab and the Excel Inquire Add-In,
and trial versions of proprietary utilities including NXPowerLite Desktop,
to assess their file size reduction behaviour.



After applying these manual guides and automatic tools to real-world Excel files, we perform a qualitative analysis of the XML code to examine the changes responsible for file size reduction. By comparing the visual integrity of the files before and after cleaning—and incrementally reverting modifications to pinpoint when visual errors disappeared—we gain insight into Excel's internal file structure and determine which cleanup operations are safe. Building on this reverse-engineering process, we identify a set of safe modifications and implement them in our own protptype tool - Excel Shrink Analyzer \cite{Johr2026ExcelShrinkAnalyzer} - using regular DOM-XML parsing for fast prototyping, to test these changes across a large number of files and validate the outcome by file size reduction and if the Spreadsheet still looks the same, to find the set of modifications that are safe and effective.

Following the prototype phase, we develop a tool applying the same modifications, but this time efficiently, using a state-machine-guided, regex-driven parser designed to clean bloated Excel files. This parser operates directly on the raw XML byte stream to overcome the limitations of conventional XML parsers when dealing with large, deeply nested structures. We evaluate the performance of this tool — referred to as Excel Shrink \cite{Johr2026ExcelShrink} — on the same Dataset a series of Excel files by comparing the loading times of pre-cleaned files versus the original files when opened with standard libraries and Microsoft Excel itself.

The scope of this work is intentionally focused on Microsoft Excel
and its Office Open XML representation.
The observed bloat patterns arise primarily from interactive, user-driven editing
practices—such as repeated formatting, copy-and-paste operations,
and manual range manipulation—that are characteristic of Excel usage.
...
Systems such as \textit{DataSpread} and \textit{Hillview} improve scalability by re-architecting the spreadsheet runtime around database-style storage and computation. Our contribution is complementary: rather than replacing Excel’s execution model, we reduce the overhead of \emph{representing} and \emph{loading} real-world workbooks by eliminating redundant OOXML content that inflates I/O, memory pressure, and parsing costs in downstream tools.
...
\textit{SheetReader} and related work accelerate \emph{general-purpose extraction} from SpreadsheetML via streaming and parallel parsing. \textit{Excel Shrink} instead performs a \emph{semantics-preserving transformation} of SpreadsheetML: it uses streaming, state-guided pattern matching to \emph{delete} redundant structures (e.g. %whitespace-only string references, 
empty styled cells, and out-of-range rows/columns) so that subsequent consumers (Excel itself and third-party libraries) face a substantially smaller parsing workload.
...
We focus on a different leverage point:
rather than replacing Excel's execution model
\textit{Excel Shrink} targets the physical representation of \texttt{.xlsx} files.
By eliminating redundant and visually inconsequential OOXML structures,
our approach reduces I/O, memory pressure, and parsing cost while preserving
full compatibility with the existing \texttt{.xlsx} ecosystem.


\textbf{Summary}
Prior work improves spreadsheet scalability either by changing the execution
architecture (e.g., database-backed spreadsheets) or by accelerating ingestion
and parsing through specialised streaming designs.
However, the community lacks systematic, reproducible evidence on
\emph{which} OOXML structures dominate file size
and \emph{which} transformations preserve visual integrity.

This paper fills that gap by
(i) empirically characterising common sources of \texttt{.xlsx} bloat, and
(ii) introducing a streaming transformation approach that reduces bloat
while retaining the \texttt{.xlsx} format for compatibility with existing
Excel-based and programmatic workflows.


\section{Methodology}

\noindent \textit{Excel Shrink} is a stream-oriented \texttt{.xlsx} cleaner designed to remove
\emph{structural bloat that is provably invisible to the user}
while preserving all layout- and semantics-relevant content.
By default, the system follows a conservative safety policy:
only XML elements whose removal cannot affect visual appearance
or formula evaluation are pruned.
Operations that may alter semantics or layout are intentionally excluded
unless explicitly enabled by the user.

\textbf{Whitespace handling policy.}
Whitespace-only cells are treated as \emph{potentially relevant} content.
Although often unintentional, such cells may affect formula semantics
(e.g., tests for non-emptiness or string concatenation)
or serve layout purposes, such as preventing text from visually overflowing
into adjacent cells.
Therefore, Excel Shrink does \emph{not} remove whitespace-only cells by default.

\textbf{System overview.}
Excel Shrink operates directly on the bytestream of the zipped OOXML container.
It parses \texttt{xl/workbook.xml} and per-sheet
\texttt{xl/worksheets/sheet*.xml} streams using regular expressions
instead of materialising full XML trees in memory,
applies workbook-specific rewriting rules derived from
\texttt{xl/styles.xml} and \texttt{xl/sharedStrings.xml},
and re-emits a compacted archive.
\textbf{Edge cases and robustness.}
A common concern with regex-based SpreadsheetML processing is whether
user-entered text could accidentally match XML patterns.
This cannot occur for valid Excel files: Excel escapes XML-like input.
As a conservative safeguard, processing aborts upon encountering
\texttt{CDATA}, which Excel itself treats as malformed.
We further rely on SpreadsheetML’s mandated element order
(violations are rejected by Excel),
while remaining agnostic to attribute ordering:
all regular expressions are written to match attributes independent of order.
Finally, prefixed/non-default namespaces (e.g., \texttt{x:worksheet}) are passed through unchanged to avoid data loss; such cases were only observed in machine-generated spreadsheets in our dataset and did not exhibit user-induced bloat.

\textbf{Formula semantics.}
Removing \emph{empty} cells, rows, or columns does not change formula semantics,
even when these coordinates are referenced.
Excel Shrink removes only their serialized representations from the worksheet XML;
upon loading, Excel materializes missing cells as empty
and evaluates references without introducing \texttt{\#REF!} errors.
...
\item \emph{Styles} (\texttt{xl/styles.xml}): provides \texttt{<cellXfs>} with fill/border/apply/protection flags.
...
\item regex-like predicates for: (i) whitespace-only cells (for optional removal), (ii) consecutive empty cells/rows, (iii) attributes irrelevant for empty rows (\emph{spans}, vendor \emph{dyDescent}, etc.), and (iv) presence of \emph{hidden} rows/columns.
\end{enumerate}
...
These operations remove visually inconsequential cells and rows,
i.e., elements that contain no content and no layout- or semantics-relevant styling.
Cells are considered visually relevant if their associated style specifies
background fills, borders, or protection attributes.
Font-only styling (e.g., font family or size) is insufficient to classify a cell
as visually relevant.
Cells marked as protected, or explicitly exempted from protection,
are conservatively preserved, as protection often encodes user intent
(e.g., preventing modification of form templates).

To identify removable cells and rows, Excel Shrink derives workbook-specific
classification rules from \texttt{styles.xml} and \texttt{sharedStrings.xml}.
From the extracted style definitions, we determine which style indices correspond to
(i) visually empty formatting, (ii) fill- or border-bearing cells, and
(iii) protected or protection-exempt cells.
Shared strings are analysed to identify whitespace-only entries
(for optional removal).
Based on these sets, we compile regular expressions that match
cells and rows that are safe to remove.
The construction of these sets and the resulting patterns
is summarised in Algorithm~\ref{alg:build-wb-patterns}.

Let \(S_{\mathrm{fbp}}\) denote the set of all \emph{fill/border/applyProtection} style indices,
i.e.\ those \texttt{xf} records where either the \texttt{fillId},
\texttt{borderId}, or \texttt{applyProtection} attribute is non-zero.
For example, if the parsed style table (\texttt{styles.xml}) contains

\[
\begin{aligned}
XFS[0].\mathrm{fillId} &= 0, \quad XFS[1].\mathrm{fillId} = 3,\\
XFS[2].\mathrm{borderId} &= 2, \quad XFS[3].\mathrm{applyProtection} = 1,
\end{aligned}
\]

then \(S_{\mathrm{fbp}}=\{1,2,3\}\).
These indices are joined into a single alternation string
\(\mathrm{FBP} = \texttt{"1|2|3"}\),
which is used to form the regular expression
\[
p_{\text{fbh}} = \code{"(?:s="(?:1|2|3)"|"hidden="1")"}
\]
matching a row attribute that either has a style with a set fill, border or applyProtection attribute, or a row attribute that is hidden used to find the last visually relevant row.
...
XFS[5].\mathrm{fillId} &= 0, \quad XFS[5].\mathrm{borderId} = 0,
\end{aligned}
\]

\noindent then \(S_{\mathrm{font}}=\{4,5\}\) is incorporated into the patterns
...
\Fn{\textsc{BuildWorkbookPatterns}($B_{\text{stylesXml}}, B_{\text{sharedStringsXml}}$)}{
  ...
  % index sets
  $S_{\mathrm{fbp}} \gets \{\, i \mid xfs[i].\mathrm{fillId}\neq 0 \lor xfs[i].\mathrm{borderId}\neq 0 \lor xfs[i].\mathrm{applyProtection} = 1 \,\}$\;
  ...
  %%% FBP + (rho3)
  \uIf{$|S_{\mathrm{fbp}}|>0$}{
    $\mathrm{FBP} \gets \Join(S_{\mathrm{fbp}}, \texttt{|})$\;
    $p_{\text{fbph}} \gets$ \code{'(?:s="(?:' + FBP + ')"|"hidden="1")'}\;
  }\Else{
    $p_{\text{fbph}} \gets$ \code{'"hidden="1"'}\;
  }
  ...
  $\mathcal{P} \leftarrow \{p_{\text{ra}},p_{\text{e}},p_{\text{fbph}},p_{\text{ws}},p_{\text{wc}}\}$ with \textsc{compile} for all non-\texttt{None}\;
...
\SetKwFunction{ConsumeMC}{ConsumeMergeCells}
  ...
  \Fn{\textsc{PreprocessSheetStreamwise}($\mathit{sheetStream}, \mathcal{P}, \mathsf{chunkSize}$)}{
    ...
    $\mathcal{D}_{col} \gets \mathrm{newRangeMap}()$;\quad \emph{column defs (map)} \;
    $m_{\max}\gets \bot$;\;
    ...
    \While{$state \neq \mathsf{FOUND\_END}$}{
      ...
      \uElseIf{$state = \mathsf{IN\_COLS}$}{
        $\langle c,\ b,\ state,\ \mathcal{D}_{col} \rangle \gets \textsc{ConsumeCols}(b,\ \mathcal{P}, \mathcal{D}_{col})$\;
...
\textbf{In column definition node}
...
\textbf{Ensuring ``Clean Cuts'' within the Streaming Buffer}
Our parser processes XML data in a streaming fashion using a working buffer that preserves XML node boundaries, ensuring that no node is split across buffer cuts. State transitions are performed only after a complete and relevant XML tag has been identified.

If no such tag can be detected, the buffer is incrementally extended with subsequent input chunks without altering the parser state or consuming already buffered content. This process is repeated until either a valid cut point is identified or the end of the input stream is reached.

If the end of the stream is reached while the parser remains inside a nested XML node and no valid closing tag can be identified, parsing is aborted and the file is skipped. This condition indicates malformed or corrupted XML; skipping such files prevents partial processing and potential data loss.

\textbf{Buffer Updates and State Progress}
...
\Fn{\textsc{SplitSheetDataRanges}($B_{\mathrm{sheetData}}$, $\mathcal{P}$)}{
  ...
  %\tcp{$patterns.p_{\mathrm{fbph}}$: compiled regex for \code{s="FBP"} or \code{hidden="1"}.}
  ...
  \ForEach{$m \in \operatorname{finditer}(patterns.p_{\mathrm{fbph}},\ B_{\mathrm{sheetData}},\ \text{start}=i_{\mathrm{cellsEnd}})$}{
...
(i) optionally identify and collapse explicit whitespace-only cells
via shared-string indices (disabled by default to preserve semantics and layout),
...
derived from \texttt{styles.xml} and \texttt{sharedStrings.xml},
as described in Section~\ref{sec:wb-patterns}:
(i) the removal of consecutive empty cells with no styling or font-only styling,
and (ii) the optional detection and removal of explicit whitespace-only cells.
In addition, we collapse consecutive hidden rows into a single sentinel row
during pre-processing.

In Excel's internal representation, rows may be marked as hidden using the attribute
\xtag{hidden="1"}.
Although such rows are visually collapsed in the spreadsheet,
their explicit representation in the worksheet XML can extend unnecessarily
over thousands or even millions of entries.
In practice, we observed worksheets in which, beyond a certain point,
all subsequent rows were marked as hidden.
Excel renders all following rows as hidden
once a single row with \xtag{hidden="1"} is encountered,
unless a later row explicitly overrides this setting.
From a rendering perspective, a long sequence of hidden rows
is therefore semantically equivalent to the presence of only the first hidden row.

To exploit this equivalence, Excel Shrink uses a regular expression
to identify and collapse consecutive hidden rows into a single representative row.
This transformation avoids expensive per-row checks
and requires no document-wide context,
making it well suited for chunk-level pre-processing.
The resulting simplification significantly reduces document size
before subsequent cleaning passes are applied.
The construction and application of the corresponding regular expression
are shown in Algorithm~\ref{alg:presd}.
...
\emph{visibly relevant attribute}---either a fill/border/protection-bearing style
(\code{s="FBP"}) or \xtag{hidden="1"}---by iterating a precompiled pattern
\mbox{$p_{\mathrm{fbph}}$}. The earliest of the next
...
\textbf{Row+cell sweep on the cells prefix}
...
\textbf{Trailing empty-rows sweep}
...
\textbf{Metrics and column merging}
...
We evaluated \textit{Excel Shrink} on 1{,}183 real-world Excel files from the German
Federal Statistical Office (\textit{Destatis})~\cite{original_destatis_xlsx_files,shrinked_destatis_xlsx_files}
and 8{,}643 real-world Excel files from \texttt{data.gov}~\cite{data_gov_xlsx_files}.
For each workbook, we verified value-level equivalence before and after cleaning
by recalculating all formulas and comparing the resulting cell values.
All values were identical, except for cells containing volatile formulas
(e.g.\ \texttt{=RAND()}, \texttt{=NOW()}, or \texttt{=CELL("filename")}),
whose semantics explicitly permit differing results across recalculations
\cite{Johr2026AnalyzeDataGovReport,Johr2026AnalyzeDestatisReport}.


\begin{figure*}
  \centering
  \includegraphics[width=\textwidth]{scripts/loading_times_size_reduction/charts/time_and_filesize_comparison_by_file.pdf}
  \caption{Loading Times and Size Reduction for the largest \textit{Destatis} and \textit{data.gov} Files Before and After applying \textit{Excel Shrink}}
  \label{fig:loading-times}
\end{figure*}

Our primary goal was to quantify the extent of data reduction achieved by
\textit{Excel Shrink} under its default, conservative configuration.
We therefore focus on metrics such as overall file size reduction,
removal of empty cells,
deletion of rows outside the range of actual data values,
and elimination of unnecessary column definitions.

Figure~\ref{fig:size-reduction-chart} presents the distribution of reduction rates
(in percentage) for individual worksheets.
To disentangle the contribution of different bloat sources,
these measurements were obtained using \textit{Excel Shrink Analyzer},
which enables optional transformations (including whitespace removal)
for diagnostic purposes.
The chart shows that some worksheets experienced reductions approaching 100\%
in file size.
In particular, one worksheet exceeded a 99.99\% reduction,
16 worksheets surpassed 99.9\%,
and 210 worksheets achieved reductions above 99\%.
Across all worksheets, the average reduction rate was 25\%.
Most of these gains were realised by removing rows that lay outside
the range of actual data values.

We further observed that optional whitespace removal yields only
comparatively small additional size reductions,
reinforcing the decision to preserve such cells by default in
\textit{Excel Shrink} given their limited contribution to bloat
and their potential impact on visual appearance or formula evaluation.

To further assess the performance improvements enabled by \textit{Excel Shrink}, we selected two particularly large workbooks from our \textit{Destatis} and \textit{data.gov} datasets \cite{destatis2023Rechnungsergebnisse, Hazari2015DobutamineQTdata}. In their original compressed \textit{Excel} (\texttt{.xlsx}) form, the files measured approximately 116~MB and 85~MB, respectively, and expanded by nearly an order of magnitude when uncompressed due to extensive sparsity and redundant formatting.

Figure~\ref{fig:loading-times} summarises runtime, file size, and peak memory
consumption before and after applying \textit{Excel Shrink}. Each file was
processed using \texttt{openpyxl}, \texttt{pandas}, \texttt{Microsoft~Excel},
\texttt{R~openxlsx}, \texttt{R~readxl+writexl}, and our \texttt{Excel~Shrink}
pipeline. For \textit{Excel Shrink}, we additionally report a single-core
execution and a variant with sheetdata splitting disabled to isolate the impact
of key algorithmic design choices.

\paragraph{Loading and Saving Times}
After shrinking, loading and saving times decrease
consistently across all libraries, often by more than an order of magnitude.
The default multi-core configuration of \texttt{Excel~Shrink} completes a full
clean-and-save cycle in only a few seconds, while the single-core variant is
slower but remains competitive, demonstrating effective parallel scalability
without relying on excessive per-core memory. Disabling sheetdata splitting
significantly increases runtime, as large sheet XML streams must be processed
more exhaustively without early truncation (Algorithm~\ref{alg:split-sheetdata}).

\paragraph{File Size Reduction}
Both workbooks shrink from approximately 116~MB and 85~MB to 3.1~MB and 4.9~MB,
corresponding to reductions of about 97.4\% and 94.2\%, respectively. These
reductions are independent of parallel execution and result solely from the
removal of visually irrelevant XML content.

\paragraph{Memory Consumption}
For the original workbooks, peak memory usage is substantial:
\texttt{openpyxl} exceeds 8~GB on the larger file, while R-based libraries peak
around 2--3~GB. After shrinking, peak memory usage drops below 1~GB across all
tools. The single-core \texttt{Excel~Shrink} variant exhibits particularly low
memory consumption, highlighting the streaming nature of the core algorithms.
In contrast, disabling sheetdata splitting increases peak memory usage, as
larger XML fragments must be retained for analysis.

Overall, these results show that \textit{Excel Shrink} achieves substantial file
size reductions while enabling faster and more memory-efficient processing
across spreadsheet ecosystems.
...
\begin{figure}[ht]
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    ...
    \caption{Before Check Performance}
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    ...
    \caption{After Check Performance}
...
\section{Future Work}
While \textit{Excel Shrink} parses \texttt{styles.xml} and
\texttt{sharedStrings.xml} for contextual correctness,
it does not yet remove unused style definitions or unreferenced shared strings.
Safely eliminating such entries—while preserving rendering and recalculation
semantics—requires global reference tracking and additional validation,

...
def generate_and_log_deleted_row(
    metrics: CleanSheetMetrics, row_attributes: RowAttributes
) -> bytes:
    metrics.deleted_row_states.append(row_attributes)
    return b""


def generate_and_log_self_closing_row(
    row_attr: bytes,
    params: CleanSheetParameters,
    metrics: CleanSheetMetrics,
    row_attributes: RowAttributes,
) -> bytes:
    metrics.kept_row_states.append(row_attributes)

    cleaned_row_attr = params.workbook_specific_patterns.pattern_remove_irrelevant_attributes_for_empty_rows.sub(
        b"", row_attr
    )
...
def generate_and_log_self_closing_cell(
    metrics: CleanSheetMetrics, cell_attributes: CellAttributes
) -> bytes:
...
def generate_and_log_deleted_cell(
    metrics: CleanSheetMetrics, cell_attributes: CellAttributes
) -> bytes:

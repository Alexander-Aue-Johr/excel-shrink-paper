...
search_duplicate_defined_name_pattern = re.compile(
    rb'<definedName\b[^>]*?name="([^"]*?)"[^>]*?localSheetId="([^"]*?)"[^>]*?>',
    re.DOTALL
)

replace_duplicate_defined_name_pattern = re.compile(
    rb'<definedName\b[^>]*?name="([^"]*?)"[^>]*?localSheetId="([^"]*?)"[^>]*?>.*?</definedName>',
    re.DOTALL
)
...
def clean_workbook(workbook_xml_bytes: bytes, params: CleanWorkbookParameters) -> bytes:
    ...
    defined_names_content, next_index, start, end = extract_xml_block(workbook_xml_bytes, b"<definedNames", b"</definedNames>")
    if defined_names_content is None:
        return workbook_xml_bytes

    matches = search_duplicate_defined_name_pattern.findall(defined_names_content)  # This line is not necessary, as we already iterate over all matches above.
    for m in matches:
    ...
    def remove_duplicate_defined_names(match: re.Match) -> bytes:
        defined_name = DefinedName(match.group(1), match.group(2))
    ...
    workbook_xml_bytes = replace_xml_block(workbook_xml_bytes, defined_names_content, start, end)

...
\title[Excel Shrink]{Excel Shrink: An Open-Source Tool for Reducing File Bloat in Excel Spreadsheets}
...
An initial sign of excessive column definitions becomes apparent when observing the horizontal scrollbar. As illustrated in Figure~\ref{fig:col_scrollbar_arrow}~\cite[Sheet: \texttt{0902\_T\_NW}]{destatis2023Foerderprogramme}, if the complete data range is visible yet the scrollbar thumb remains extremely narrow relative to the scrollable area, it suggests that numerous redundant column definitions extend well beyond the actual data range.
By scrolling into these regions, users may notice that among columns with uniform widths, some columns occasionally have differing widths, as shown in Figure~\ref{fig:col_s_arrow}. Such anomalies further indicate the presence of unnecessary columns contributing to file bloat.
...
\begin{figure}[ht]
  ...
  \end{subfigure}
  \caption{Indicators of unnecessary column definitions extending beyond the actual data range.}
...
To remove redundant column definitions, we can implement a procedure as outlined in Algorithm~\ref{alg:remove_redundant_columns}. This algorithm identifies the actual right boundary of data and cell formats, locates redundant column definitions, and adjusts the last column definition to account for the removed columns.

\begin{algorithm}
  \caption{Removing Redundant Column Definitions}
  \label{alg:remove_redundant_columns}
  \begin{algorithmic}[1]
    \STATE \textbf{Input:} Worksheet XML structure with column definitions
    \STATE \textbf{Output:} Modified worksheet XML without redundant column definitions
    
    \STATE Identify the actual right boundary of the data range and cell formats.
    \STATE Find the first redundant column definition, record its \texttt{min} value as \textit{savedMin}.
    \STATE Remove all redundant column definitions except the last one.
    \STATE Overwrite the \texttt{min} value of the last remaining redundant column definition with \textit{savedMin}.

    \STATE \textbf{return} Modified XML
  \end{algorithmic}
\end{algorithm}
...
A similar issue occurs with \texttt{Print\_Titles} and \texttt{\_xlnm.Print\_Titles}. In these cases, \textit{Excel} cannot simply rename the entries without causing duplicates. Instead, it displays a dialog box prompting the user to select a new name for each redundant \textit{Defined Name} upon file opening.

If the user renames the first entry and clicks \emph{OK}, the dialog reappears for each subsequent redundant entry. This process repeats until all duplicates are resolved. The user cannot reuse the same name for different \textit{Defined Names} on the same worksheet, which further complicates the process. Moreover, this dialog prevents automated file opening via VBA to leverage tools like the \textit{Inquire} Add-in or the \textit{Excel Performance} tab for batch file size reduction, since the dialog interrupts any automation.

In the examined files, we observed up to 60 redundant \texttt{Print\_Area} and 55 redundant \texttt{Print\_Titles} entries. Renaming these entries confers no actual benefit, as the relevant print areas and titles are already correctly migrated. Users may believe they are repairing \textit{Print Areas} or \texttt{Print Titles}, but in reality, they are only renaming redundant \textit{Defined Names}. To mitigate this issue and spare users from manually renaming entries, \textit{Excel Shrink} automatically removes these redundant definitions.

\subsection{Reporting Reduction Results}

To quantify the impact of each reduction step—such as removing whitespace, empty cells, columns, and rows—we compare file sizes before and after each optimization. Specifically, we convert the worksheet’s XML structure into a string representation and record its length. After performing a reduction step, we measure the string length again and calculate the difference. The resulting percentage indicates how much size reduction that particular step achieved. These results are recorded in a CSV file for further analysis~\cite{excel_shrink_results_csv}.
...
In this evaluation, we processed all \textit{Excel} files obtained from the German Federal Statistical Office (\textit{Destatis})~\cite{original_destatis_xlsx_files,shrinked_destatis_xlsx_files}. Detailed information on how these files were acquired via a custom web scraping approach can be found in Appendix~\ref{sec:scraping_destatis}.



Our primary objective was to measure the degree of data reduction achieved by \textit{Excel Shrink}, focusing on metrics such as overall size reduction, removal of empty cells, elimination of whitespace cells, deletion of rows outside the value range, and removal of unnecessary columns. Figure~\ref{fig:size-reduction-chart} presents the distribution of reduction rates (in percentages) for individual worksheets across all these categories. Some worksheets approached a 100\% reduction in file size. In particular, one worksheet exceeded 99.99\%, 16 worksheets surpassed 99.9\%, and 210 worksheets achieved reductions above 99\%. On average, worksheets were reduced by 25\%. Most of these improvements resulted from removing rows that lay outside the actual data range.

To further assess performance improvements, we identified the largest available \textit{Destatis} file~\cite{destatis2023Rechnungsergebnisse}. Before processing, this file measured 85.2~MB in its compressed \textit{Excel} (.xlsx) format and expanded to 911~MB uncompressed. After applying \textit{Excel Shrink}—a process taking a median of 484 seconds over 10 runs—the compressed file size dropped to 2.76~MB, with an uncompressed size of just 20.6~MB~\cite{destatis2023RechnungsergebnisseShrunk}.

Subsequent tests were performed using both the Dart \texttt{excel} package~\cite{excel_shrink_dart_test, excelPackage} and the Python \texttt{OpenPyXL} package~\cite{open_large_xlsx_file_test, openpyxl_documentation}. Prior to shrinking, the Dart \texttt{excel} package required 794.7 seconds on its first run to open and read values, then failed with an out-of-memory exception on the second run~\cite{excel_shrink_dart_test_results_original}. The \texttt{OpenPyXL} package required a median of 301 seconds over 10 runs~\cite{excel_shrink_openpyxl_test_results_original}.

In contrast, after applying \textit{Excel Shrink}, the Dart \texttt{excel} package successfully ran 10 consecutive tests without error, with a median time of 29.8 seconds~\cite{excel_shrink_dart_test_results_shrunk}, and \texttt{OpenPyXL} required a median of 5.2 seconds~\cite{excel_shrink_openpyxl_test_results_shrunk}, demonstrating a substantial performance improvement. Figure~\ref{fig:comparison_of_processing_times_before_and_after_excel_shrink} shows these results, highlighting the effectiveness of \textit{Excel Shrink} in reducing processing times.

\begin{figure*}
  \centering
  \includegraphics[width=\textwidth]{images/charts/comparison_of_processing_times_before_and_after_excel_shrink.pdf}
  \caption{Comparison of processing times before and after applying \textit{Excel Shrink}.}
  \label{fig:comparison_of_processing_times_before_and_after_excel_shrink}
\end{figure*}



Our evaluation demonstrates that \textit{Excel Shrink} effectively eliminates redundant, empty, or irrelevant data across a diverse set of \textit{Excel} worksheets. Significant reductions—exceeding 90\% in several cases—were achieved in sheets containing a high proportion of empty cells, or irrelevant rows and columns. These results highlight the robustness of our reduction methods in processing diverse data formats and structures. Moreover, the performance improvements in file opening times underscore the practical benefits of using \textit{Excel Shrink} to enhance data processing efficiency.

\section{Experimental Setup}
\label{sec:experimental-setup}

To evaluate the behavior of \textit{Excel} regarding how the files are interpreted and saved, as well as to test the behavior of the \textit{Add-in} and the new \textit{Check Performance} feature, we tested with two distinct versions of \textit{Excel} from \textit{Microsoft Office}: \textit{Microsoft Office LTSC Professional Plus 2021} and \textit{Microsoft Excel for Microsoft 365 MSO (Version 2409 Build 16.0.18025.20030) 64-bit}. These versions were selected to evaluate compatibility and performance across different Excel environments.Our test machine was an \textit{Intel Core i5-10210U} quad-core CPU desktop machine running at 1.60~GHz with 16~GB of RAM.

\section{Outlook}

While \textit{Excel Shrink} can significantly reduce file sizes and thereby expedite the opening of \textit{Excel} files through libraries like \texttt{OpenPyXL} or the Dart \texttt{excel} package, there is still room for further optimization. The current implementation relies on an XML DOM parser, which can be replaced with a StAX or SAX parser to lower memory usage and improve processing speed. Such a streaming-based parser would consume far less memory, allowing multiple processes or threads to operate on different worksheets in parallel, thus further reducing the overall processing time.

...
def find_and_slice_after_tag_range(
    buffer: bytes, tag_start, tag_end, search_backwards: bool = False
) -> Optional[SliceAtTagResult]:
    if search_backwards:
...
def find_and_slice_at_tag(
    buffer: bytes,
    tag: bytes,
    where_to_slice: WhereToSlice,
    ...
    if where_to_slice == WhereToSlice.before_tag:
        return SliceAtTagResult(
            until_tag=buffer[:index_tag], after_tag=buffer[index_tag:]
        )
    else:
        after_tag_name = index_tag + len(tag)
        return SliceAtTagResult(
            until_tag=buffer[:after_tag_name], after_tag=buffer[after_tag_name:]
        )


def find_and_slice_before_tag(
    buffer: bytes, tag_start_pattern, search_backwards: bool = False
) -> Optional[SliceAtTagResult]:
    return find_and_slice_at_tag(
        buffer, tag_start_pattern, WhereToSlice.before_tag, search_backwards
    )


def find_and_slice_after_tag(
    buffer: bytes, tag_start_pattern, search_backwards: bool = False
) -> Optional[SliceAtTagResult]:
    return find_and_slice_at_tag(
        buffer, tag_start_pattern, WhereToSlice.after_tag, search_backwards
    )

...
\item \textbf{(RQ3)} Does precleaning bloated Excel files with such a method significantly improve overall loading time (\emph{i.e.}, cleaning + loading) compared to loading the original file directly in Excel or popular libraries?
...
\begin{itemize}
  ...
  \item \textbf{2:} \lstinline|(:\d{3,}|4[3-9]|4\d{1})| \\
    ...
    \begin{itemize}
      ...
      \item \lstinline|4\d{1}| captures additional two-digit rows starting with 4 when applicable. Here, \lstinline|\d{1}| should be written as \lstinline|4\d\{\1\}|.
    \end{itemize}
...
\subsection{Vergleich mit anderen Werkzeugen zum reduzieren von Bloat}

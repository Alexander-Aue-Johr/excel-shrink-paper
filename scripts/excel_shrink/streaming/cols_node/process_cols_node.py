...
def extract_column_definitions_and_apply_fill_border_column_styles(
    col_data: bytes | None, params: CleanSheetParameters
) -> ColumnRangeMap:
    if col_data is None:
    ...
    for style_bytes, min_index_bytes, max_index_bytes, hidden in cols_style_matches:
        ...
        style = (
            params.workbook_specific_patterns.xfs_by_bytestring[style_bytes]
            if style_bytes
            else None
        )
...
def process_cols_node(
    buffer: bytes, params: CleanSheetParameters
) -> CollectColsResult | None:
    found_closing_tag = find_and_slice_before_tag(buffer, b"</cols>")
    if found_closing_tag is None:
        return None

    column_definitions = extract_column_definitions_and_apply_fill_border_column_styles(
        found_closing_tag.until_tag, params
    )

...
% Safe inline "code" for regex / byte literals (prints backslashes verbatim)
...
% Tiny helpers to keep math vs. text separated
...
\[
XFS[0].\mathrm{fillId}=0,\quad
XFS[1].\mathrm{fillId}=3,\quad
...
\Fn{\textsc{BuildWorkbookPatterns}($B_{\text{stylesXml}}, B_{\text{sharedStringsXml}}$)}{
  $xfs \gets \mathrm{parseCellXfs}(B_{\text{stylesXml}})$\;
  $S_{\mathrm{fb}} \gets \{\, i \mid xfs[i].\mathrm{fillId}\neq0 \lor xfs[i].\mathrm{borderId}\neq0 \,\}$\;
  $S_{\mathrm{font}} \gets \{\, i \mid xfs[i].\mathrm{fillId}=6 \,\}$\;
...
\Fn{\textsc{BuildWorkbookPatterns}($B_{\text{stylesXml}}$, $B_{\text{sharedStringsXML}}$)}{
  ...
  $W \leftarrow \IdxWhitespaceSI(B_{\text{sharedStringsXML}})$\;
  ...
  \uIf{$|S_{\mathrm{font}}|>0$}{
    $\mathrm{OFS} \leftarrow \Join(S_{\mathrm{font}}, \texttt{|})$\;
    $p_{\text{ra}} \leftarrow$ \code{'\s*(\bspans="[^"]+?"|' + }\\
    \code{   '\b[^\s:]*?:dyDescent="[^"]*"|' + } \\
    \code{   '\bs="(?:' + OFS + ')")'}\;
  ...
  $R \leftarrow \{p_{\text{ra}},p_{\text{e}},p_{\text{fbh}},p_{\text{ws}},p_{\text{wc}}\}$ with \textsf{compile} for all non-\texttt{None}\;


  \Return $\textsf{Patterns}(\mathrm{xfs}, R)$\;
}
...
\begin{algorithm}
  ...
  \SetKwFunction{PreSD}{$\mathrm{preProcessSheetData}$}
  \SetKwFunction{TrackMC}{$\mathrm{trackMaxMerge}$}

  \SetKwFunction{Preprocess}{$\mathrm{PreprocessSheetStreamwise}$}
  \SetKwProg{Fn}{Function}{:}{}
...
\Fn{\textsc{PreprocessSheetStreamwise}($\mathit{sheetStream}, \mathcal{P}, \mathsf{chunkSize}$)}{
  ...
  $\phi_{\text{cols}} \gets \mathrm{parseColDefs}(x,\mathcal{P})$;\quad
  $\phi_{\text{sd}} \gets \mathrm{preProcessSheetData}(x,\mathcal{P})$;\quad
  $\phi_{\Delta} \gets \mathrm{trackMaxMerge}(x)$;\;

  \While{$state \neq \mathsf{FOUND\_END}$}{
 
    \eIf{$state=\mathsf{IN\_WORKSHEET}$}{

      \If{$tag=\texttt{<cols>}$}{
        $B_{\text{preCols}} \gets B_{\text{preCols}} \,\|\, \mathrm{takeUntil}(b, tag)$;\;
        $state \gets \mathsf{IN\_COLS}$;\;
      }
      \ElseIf{$tag=\texttt{<sheetData>}$}{\dots}
      \ElseIf{$tag=\texttt{<mergeCells>}$}{\dots}
      \Else{\textbf{break}}
    }{
      \uIf{$state=\mathsf{IN\_COLS}$}{
        $(chunk,b,state,\mathcal{D}) \gets \mathrm{consumeUntil}(b,\texttt{</cols>},\phi_{\text{cols}})$;\;
        $B_{\text{cols}} \gets B_{\text{cols}} \,\|\, chunk$;\;
      }
      \uElseIf{$state=\mathsf{IN\_SHEETDATA}$}{\dots}
      \uElseIf{$state=\mathsf{IN\_MERGECELLS}$}{\dots}
    }
  }

  \KwRet~$\langle
     \mathrm{Preprocessed}:\langle B_{\text{preCols}}, B_{\text{cols}}, B_{\text{preSD}}, B_{\text{sheetData}},
                              B_{\text{preMC}}, B_{\text{mergeCells}}, B_{\text{tail}}\rangle,\;
     \mathrm{Metrics}:\langle \mathcal{D}, m_{\max}\rangle
  \rangle$\;
}
...
\Fn{\textsc{PreprocessSheetStreamwise}($\mathit{sheetStream}, \mathcal{P}, \mathsf{chunkSize}$)}{
  ...
  $B_{\text{preCols}}, B_{\text{cols}}, B_{\text{preSD}}, B_{\text{sheetData}},
    B_{\text{preMC}}, B_{\text{mergeCells}}, B_{\text{tail}} \gets \epsilon$;\;
  $\mathcal{T} \gets \varnothing$;\quad $\mathcal{D} \gets \mathrm{newRangeMap}()$;\quad $m_{\max}\gets \bot$;\;

    $state \gets \texttt{IN\_WORKSHEET}$;\quad $buf \gets \epsilon$;\;
    \textbf{initialise accumulators: }
      $B_{\mathrm{preCols}},\ B_{\mathrm{cols}},\ B_{\mathrm{preSD}},\ B_{\mathrm{sheetData}},\ B_{\mathrm{preMC}},\ B_{\mathrm{mergeCells}},\ B_{\mathrm{tail}}$;\;
    ...
    $\phi_{\mathrm{cols}}(x) \gets \mathrm{parseColDefs}(x,\ patterns)$;\;
    $\phi_{\mathrm{sd}}(x) \gets \mathrm{preProcessSheetData}(x,\ patterns)$;\;
    $\phi_{\mathrm{mc}}(x) \gets \mathrm{trackMaxMerge}(x)$;\;

    \While{$state \neq \mathsf{FOUND\_END}$}{
      $b \gets b \,\|\, \mathrm{read}(\mathit{sheetStream}, \mathsf{chunkSize})$;\;

      \uIf{$state = \mathsf{IN\_WORKSHEET}$}{
        $remainingTags \gets \mathcal{O} \smallsetminus processedTags$\;
      $\langle tag, span\rangle \gets \mathrm{nextTag}(b, \mathcal{T})$;\;
      ...
      \uElseIf{$state = \mathsf{IN\_COLS}$}{
        ...
        \textsf{appendInPlace} \!($B_{\mathrm{cols}},\ chunk$)\;
      }
      ...
      \uElseIf{$state = \mathsf{IN\_SHEETDATA}$}{
        ...
        \textsf{appendInPlace} \!($B_{\mathrm{sheetData}},\ chunk$)\;
      }
      ...
      \uElseIf{$state = \mathsf{IN\_MERGECELLS}$}{
        ...
        \textsf{appendInPlace} \!($B_{\mathrm{mergeCells}},\ chunk$)\;
      }

      \Else{
        \textsf{appendInPlace} \!($B_{\mathrm{tail}},\ \textsf{rest}(buf)$)\; \textbf{break}\;
      }
    }
...
\Return $\textsf{Preprocessed}\left(
  \vcenter{\hbox{$
    \begin{alignedat}{1}
      ...
      &B_{\mathrm{preMC}},\ B_{\mathrm{mergeCells}},\ B_{\mathrm{tail}},\\
...
\Fn{splitSheetDataRanges($sheetData$, $\mathcal{P}$)}{
  ...
  $i_{\mathrm{lastC}} \gets \textsf{rfind}(sheetData,\ \code{"<c "})$\;
  \If{$i_{\mathrm{lastC}} = -1$}{
    $i_{\mathrm{cellsEnd}} \gets -1$\;
  }
  \Else{
    $j \gets \textsf{find}(sheetData,\ \code{"</row>"},\ i_{\mathrm{lastC}})$\;
  ...
  \ForEach{$m \in \textsf{finditer}(patterns.p_{\mathrm{fbh}},\ sheetData,\ \text{start}=i_{\mathrm{cellsEnd}})$}{
  ...
  $j_{\mathrm{close}} \gets \textsf{find}(sheetData,\ \code{"</row>"},\ i_{\mathrm{visAttr}})$\;
  $j_{\mathrm{self}}  \gets \textsf{find}(sheetData,\ \code{"/>"},\ i_{\mathrm{visAttr}})$\;
  ...
  \Else{
    \KwRet \textsf{error}(\code{"no </row> or /> after last anchor"})\;
  }
  ...
  \uIf{$i_{\mathrm{cellsEnd}} = -1$}{
    ...
    $\textit{cellsRange} \gets sheetData[0 : i_{\mathrm{cellsEnd}}]$\;
  }
  ...
  \uIf{$i_{\mathrm{rowsEnd}} = -1$}{
    ...
    $\textit{rowsRange} \gets x[i_{\mathrm{rowsStart}} : i_{\mathrm{rowsEnd}}]$\;
  }

  \BlankLine
  \KwRet $(\textit{cellsRange},\ \textit{rowsRange})$\;
}
\end{algorithm}
...
%\subsection{Row/Cell Cleaning Semantics}
%
%
%
...
\Fn{cleanRowsAndCellsInRange($B_{\text{row}}$, $\mathcal{P}$, $\mathcal{M}$, $XFS$)}{
  %\tcp{$\mathcal{M}$: mutable per-sheet state; $params$: xfs/columns.}
  \KwRet \Sub($B_{\text{row}}$, $p_{\text{row}}$,
    \ReplaceRow($\cdot$, $\mathcal{P}$, $\mathcal{M}$, $XFS$))\;
}
...
\Fn{ReplaceRow($m$, $\mathcal{P}$, $\mathcal{M}$, $XFS$)}{
...
\Fn{CleanCells($rowBytes$, $\mathcal{P}$, $\mathcal{M}$, $XFS$)}{
...
\S\ref{sec:wb-patterns}) and update a small, per-sheet metrics state $\mathcal{M}$ used by later
...
\Fn{cleanEmptyRowsFromAfterCellRange($x$, $\mathcal{P}$, $\mathcal{M}$, $XFS$)}{
...
\Fn{ReplaceEmptyRow($m$, $\mathcal{M}$, $XFS$)}{
...
those are then used to merge extraneous \xtag{<col>} ranges, as outlined in
\S\ref{sec:columns}.
...
\begin{lstlisting}[
  ...
  label={lst:row-65533},
  basicstyle=\ttfamily\footnotesize,  % kleiner: \scriptsize geht auch
  frame=none,                         % kein Rahmen
  breaklines=true,                    % Zeilen umbrechen
  columns=fullflexible,               % bessere Laufweite
  showstringspaces=false              % keine ␣-Markierungen
]

...
These inefficiencies not only increase storage requirements but also degrade \textit{Excel}'s performance. Larger files take longer to open, save, and modify \cite{mack2018characterizing, rahman2020benchmarking}. In enterprise environments, this can lead to significant delays when handling shared workbooks or automating data processing tasks through scripts and applications that interface with \textit{Excel}. The excessive size of the files puts a strain on memory and processing resources, leading to a slower user experience and increased frustration in collaborative settings where files are frequently shared or integrated with other systems. These inefficiencies, therefore, represent a considerable challenge for users working with large datasets or managing multiple \textit{Excel} files.
  ...
  Our research was motivated by the need to understand and address these shortcomings. We set out to identify the specific causes of file bloat in \textit{Excel} files and to develop a solution that overcomes the limitations of existing tools. This involved:
  ...
  \begin{itemize}
      ...
      \item \textbf{Evaluating Existing Tools:} We examined the limitations of existing solutions, including analyzing the damage they can cause to spreadsheets' visual layouts by inspecting the altered XML structures after processing.
  ...
  \begin{itemize}
      \item We provide a comprehensive analysis of the root causes of file bloat in \textit{Destatis} \textit{Excel} files, uncovering previously undocumented issues such as unnecessary column definitions.
      ...
      \item We develop \textit{Excel Shrink}, an open-source tool that surpasses existing solutions by effectively reducing file sizes while preserving the visual formatting of \textit{Excel} files. For users who prefer a user-friendly interface without the need to install \textit{Python} or use command-line tools, we developed \textit{Excel Shrink UI}~\cite{excel-shrink-ui}. 
      \item We evaluate \textit{Excel Shrink} on a comprehensive dataset of \textit{Destatis} \textit{Excel} files. Our results demonstrate that \textit{Excel Shrink} can reduce file sizes by up to 99.99\% without compromising the usability or appearance of the \textit{Excel} files
  \end{itemize}

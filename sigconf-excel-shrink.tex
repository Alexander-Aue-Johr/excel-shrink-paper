...
\begin{abstract}
  Excel files are widely utilized as a format for data exchange across various domains. However, as spreadsheets grow in complexity, significant challenges arise regarding file size and performance management \cite{swidan2016improving, mack2018characterizing}.
  Wir untersuchten excel dateien unter anderem von dem German Federal Statistical Office (\textit{Destatis}) und beobachteten sehr lange ladezeiten sowohl in microsoft excel als auch in softeare packages zum auslesen von excel dateien, bis hin zu programmabstützen bedingt durch out of memory exceptions während des ladens dieser dateien. Wir haben den quellcode dieser Dateien untersucht. Die dateien enthielten formatdefinitionen die für den nutzer unsichtbar sind. Diese Formatdefinitionen namen orders of magnitude mehr speicherplatz ein, als die eigentlichen daten.
  While some proprietary tools attempt to address this issue, they are closed-source and do not disclose the underlying causes of file bloat. Moreover, these tools often fail to fully resolve the problem or introduce new issues, such as altering the visual apearance of spreadsheets.
  In this paper, we present a comprehensive analysis of the root causes of Excel file bloat, documenting issues, die von proprietary tools bereits behoben werden aber undokumentiert blieben, aber außerdem uncovering issues die previously nicht von proprietary tools behoben wurden. Wir analisierten auch, aus welchem grund proprietary tools die visuelle Integrität der Dateien beeinträchtigen haben.
  We developed an open-source tool, Excel Shrink, welches effektiv die gleichen root causes of Excel file bloat behebt als auch die root causes of Excel file bloat behebt, die bisher kein proprietary tools entfernt hat.
  Um unser Tool zu testen haben wir alle Excel Dateien vom German Federal Statistical Office (\textit{Destatis}) heruntergeladen und mit unserem Tool optimiert. Wir haben die Dateigrößen vor und nach der Optimierung verglichen und festgestellt, dass unser Tool die Dateigrößen um bis zu 99.99\% reduzieren kann, ohne die visuelle Integrität der Dateien zu beeinträchtigen.
  \end{abstract}
...
Excel files are widely utilized as a format for data exchange across various domains, including finance, statistics, business, and research \cite{panko1998we}. Their versatility lies in \textit{Excel}'s capability to store tabular data, perform complex calculations, automate workflows using macros, and visually represent data through charts and graphs. This flexibility makes \textit{Excel} indispensable in many organizations. However, as spreadsheets grow in complexity, significant challenges arise regarding file size and performance management \cite{swidan2016improving, mack2018characterizing}.

The German Federal Statistical Office (\textit{Destatis}) publishes a multitude of \textit{Excel} sheets containing statistical data~\cite{destatis}. Users of these datasets have reported that many of these files are excessively large, taking several seconds to open and causing performance issues in both manual and automated data processing tasks. In extreme cases, applications attempting to read these files may crash due to memory overload. Understanding the underlying causes of this file bloat is non-trivial due to the complex and opaque internal structure of \textit{Excel} files.  
Existing tools and features, such as the \textit{Excel Performance} tab, \textit{NXPowerLite Desktop}, and the \textit{Inquire} Add-in, aim to reduce \textit{Excel} file sizes. However, these tools have notable limitations:

\begin{itemize}
    \item \textbf{Closed Source and Lack of Transparency:} All existing tools are proprietary and do not disclose the underlying methods they use to address file bloat. This opacity prevents users and researchers from understanding the root causes of the problem and limits the potential for further optimization.
    \item \textbf{Incomplete Solutions:} These tools often fail to fully resolve the issue, neglecting certain types of redundant content such as unnecessary column definitions. This means that significant portions of file bloat remain unaddressed.
    \item \textbf{Alteration of Visual Appearance:} In some cases, particularly with the \textit{Excel Performance} tab, these tools significantly alter the visual appearance of spreadsheets. For users who need to preserve the original formatting, such changes are unacceptable.
    \item \textbf{???} Die Tools können nicht automatisiert alle Dateien auf einmal optimieren. Excel Funktionen bzw. Plugins wie Inquire und der Performance Tab erfordern jede Datei einzeln zu öffnen. Ein automatisiertes anwenden der Funktionen bzw. Plugins mittels VBA Scribt funktioniert nicht, da manche redundanzen in Excel Dateien dafür sorgen, dass Excel beim öffnen der Datei den Nutzer mit dialogen auffordert, die redundanzen manuell zu beheben und somit die Automatisierung stoppt. NXPowerLite Desktop limitiert dagegen die Anzahl der zu optimierten Dateien künstlich auf 20 Dateien.
\end{itemize}

Our research was motivated by the need to understand and address these shortcomings. We set out to identify the specific causes of file bloat in \textit{Destatis} \textit{Excel} files and to develop a solution that overcomes the limitations of existing tools. This involved:
\begin{itemize}
  \item \textbf{Analyzing the Root Causes of File Bloat:} We performed a systematic investigation into why \textit{Excel} files become excessively large, focusing on hidden inefficiencies such as redundant column definitions and unused cell formats that have not been previously documented in the scientific literature.
  \item \textbf{Reverse Engineering Existing Tools:} Without access to the source code of existing proprietary tools, we had to reverse-engineer their behavior by analyzing the differences in XML structures before and after processing. This helped us understand their limitations and unintended consequences, such as altering the visual appearance of spreadsheets.
  \item \textbf{Developing a Superior Solution:} We created \textit{Excel Shrink}, an open-source tool that effectively reduces file sizes by addressing all identified causes of file bloat without altering the spreadsheets' appearance.
\end{itemize}

Our contributions can be summarized as follows:

\begin{itemize}
    \item We provide the first comprehensive analysis of the root causes of file bloat in \textit{Destatis} \textit{Excel} files, uncovering previously undocumented issues such as unnecessary row, column and cell definitions, whitespace cells and Print Areas and Print Titles.
    \item We reveal the limitations of existing proprietary tools, explaining why they fail to fully address file bloat and how they can unintentionally alter the visual appearance of spreadsheets.
    \item We develop \textit{Excel Shrink}, an open-source tool that surpasses existing solutions by effectively reducing file sizes while preserving the visual formatting of \textit{Excel} files.
    \item Wir integrieren \textit{Excel Shrink} in eine Grafische Nutzeroberfläche \textit{Excel Shrink UI}, die es jedem Nutzer ermöglicht, ohne die Notwendigkeit von \textit{Python} oder Kommandozeilenwerkzeugen, seine \textit{Excel} Dateien zu optimieren.
    \item We evaluate \textit{Excel Shrink} on a comprehensive dataset of \textit{Destatis} \textit{Excel} files, demonstrating substantial improvements in file size reduction.
\end{itemize}

By addressing the previously unexplored problem of hidden redundancies in \textit{Excel} files and providing a transparent, open-source solution, our work fills a significant gap in the field of spreadsheet optimization.
...
\section{Analyzing Redundant Content in Excel Files}
\label{sec:analysis}
...
\begin{itemize}
    ...
    \item \textbf{Print Areas and Print Titles:} Some Print Areas and Print Titles may be defined via names that were used by older versions of \textit{Excel} but were not migrated to the current version. These definitions remain in the file, consuming space without serving any purpose and forcing the user to manually rename them upon opening the file because they cause name conflicts, which makes batch processing these files with VBA impossible because each dialog halts the automation.
\end{itemize}
...
Several tools and techniques have been developed to mitigate \textit{Excel} file bloat, including proprietary software like \textit{NXPowerLite Desktop}, the \textit{Excel Inquire} Add-in, and the built-in \textit{Check Performance} feature in recent versions of \textit{Excel}. However, these tools have notable limitations.

\subsection{NXPowerLite Desktop}

\textit{NXPowerLite Desktop} is a commercial program primarily designed to compress embedded images in \textit{Excel} files~\cite{nxpowerlite_desktop}. While it can remove some empty cells, it ignores other sources of file bloat such as cells containing only whitespaces and redundant column definitions, Print Areas and Print Titles. Moreover, being closed-source, erlaubt es uns nicht das beheben übrigen file bloat ursachen zu contributen oder die künstliche limitierung auf maximal 20 Dateien zu umgehen.

\subsection{Excel Inquire Add-in}

The \textit{Excel Inquire} Add-in, available only for certain enterprise versions of \textit{Excel}, can remove some redundant content~\cite{o2014excel}. However, it lacks batch processing capabilities, making it impractical for large datasets. Additionally, it fails to identify and remove many redundant rows and columns, particularly those with custom properties like hidden rows or custom heights. Außerdem ignoriert es whitespace cells und  Print Areas and Print Titles.

\subsection{Excel Check Performance}

The \textit{Check Performance} feature introduced in \textit{Excel} for Web and recent desktop versions can achieve significant file size reductions~\cite{shirolkar2022check,shirolkar2024make}. However, it often alters the visual appearance of spreadsheets by removing cells and rows that impact formatting and layout. This side effect is unacceptable in contexts where preserving the visual integrity of the spreadsheet is crucial. Furthermore, it does not address certain types of redundancies, such as unnecessary column definitions, which we have identified as significant contributors to file size.

\subsection{Limitations of Existing Tools}

None of these existing tools address all types of redundant content. By identifying these gaps, we recognized the need for a solution that not only reduces file sizes more effectively but also preserves the visual integrity of the spreadsheets. Our work fills this gap by providing a detailed analysis of the root causes of \textit{Excel} file bloat and an open-source tool that effectively addresses these issues without compromising the visual layout of spreadsheets.
